...
SingleNode* defectiveList = NULL;
DoubleNode* repairedList = NULL;
...
void moveToRepairedList(char* car) {
    ...
    if (prev == NULL) defectiveList = temp->next;
    ...
    if (repairedList != NULL) repairedList->prev = newNode;
